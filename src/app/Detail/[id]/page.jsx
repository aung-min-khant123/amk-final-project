...
export default function ProductDetails() {
  ...
  const getPrice = () => {
    if (isCake) return product.price;
    if (!size) return 0;
  ...
  const hasTemperature = product.options?.temperature;
    ...
    <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-2 ">
      ...
      <div className="">
        ...
        <div className="mx-5 my-6 bg-white/20 backdrop-blur-md backdrop-brightness-125 border border-white/20 rounded-md ">
            ...
            <>
              <div className="my-4">
                <div className="flex justify-center gap-4 my-5">
                  <h3 className="text-center text-xl font-bold font-mono text-stone-400 underline">
                    Hot or Iced:
                  </h3>
                  <img src="/images/hot.png" alt="hot" className="w-7 h-7 object-contain"/>
                  <img src="/images/ice-cube.png" alt="iced" className="w-7 h-7 object-contain"/>
                </div>

                <div className="flex justify-center gap-3 mt-3">
                  {hasTemperature.map((temp) => (
                    <button
                      key={temp}
                      onClick={() => setTemperature(temp)}
                      className={`flex justify-center items-center gap-2 px-4 py-1 rounded-md font-bold ${
                        temperature === temp
          ...
          <div>
            <h2 className="text-center font-bold font-mono text-stone-400 py-2 underline">
              Eat here & Take away
            </h2>
            <div className="flex justify-center gap-2.5 py-3 ">
              {["Eat here", "Take away"].map((c) => (
                <button
                  key={c}
                  onClick={() => setChoice(c)}
